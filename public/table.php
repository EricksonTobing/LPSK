<?php
  ...
  $t = $_GET['t'] ?? 'permohonan';
  if (!isset($tables[$t]) || $t==='users') { http_response_code(404); exit('Not Found'); }
  ...
  // Fix for the POST handler section:
    ...
    try {
      if ($action==='create' || $action==='update') {
        ...
        foreach (array_keys($colLabels) as $c) {
          // Skip joined columns (they come from other tables)
          $isJoined = false;
        ...
        if (in_array($t, ['layanan', 'penelaahan']) && empty($data['no_reg_medan'])) {
        ...
        if ($action==='create') {
          ...
          $id = $_POST[$pk] ?? null;
          if (!$id) { $_SESSION['error'] = "ID required"; redirect("table.php?t=$t"); }
          $set = [];
          $params = [];
  ...
  // Function to determine input type based on column name and data
  function get_input_type($column, $value = '') {
    ...
    if (strpos($column, 'tanggal') !== false || strpos($column, 'tgl') !== false || strpos($column, 'masa') !== false || strpos($column, 'waktu') !== false) {
      return ['type' => 'date'];
    }
  ...
  if ($q !== '' && $searchable) {
    ...
    foreach ($searchable as $s) { 
      // Handle joined columns in search
      $isJoined = false;
      $searchTable = $t;
  ...
  foreach ($filters as $f) {
    if ($val = trim((string)($_GET[$f] ?? ''))) {
      // Handle joined columns in filters
      $isJoined = false;
      $filterTable = $t;
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden mb-8">
  ...
  <div>
    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?= e($f) ?></label>
    <?php 
    ...
    <?php if ($input_type['type'] === 'select'): ?>
      <select name="<?= e($filterName) ?>" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
        ...
        <?php foreach ($input_type['options'] as $option): ?>
          <option value="<?= e($option) ?>" <?= ($_GET[$filterName] ?? $_GET[$f] ?? '') === $option ? 'selected' : '' ?>>
            <?= e($option) ?>
          </option>
        <?php endforeach; ?>
      </select>
    <?php else: ?>
      <input type="<?= e($input_type['type']) ?>" name="<?= e($filterName) ?>" value="<?= e($_GET[$filterName] ?? $_GET[$f] ?? '') ?>"
...
<?php endforeach; ?>
  ...
  <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
    ...
    <div class="inline-flex mt-2 xs:mt-0">
      ...
      <?php $nextPage = $page < ceil($total/$per) ? $page + 1 : $page; ?>
      
      <?php $prevParams = array_merge($_GET, ['page' => $prevPage]); ?>
      <?php $nextParams = array_merge($_GET, ['page' => $nextPage]); ?>
    ...
    <div class="mt-3">
      ...
      <form id="editForm" method="post" class="mt-4 space-y-4 max-h-96 overflow-y-auto pr-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <?php foreach ($colLabels as $col => $label): 
            // Skip joined columns in edit form
            $isJoined = false;
            foreach ($joins as $joinTable => $joinInfo) {
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mt-6">
  ...
  <form method="post" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <?php foreach ($colLabels as $col => $label): 
      ...
      // Modifikasi khusus untuk link_berkas_permohonan
